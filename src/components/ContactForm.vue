<template>
  <Form @submit="submitContact" :validation-schema="contactFormSchema">
    ...
    <div class="form-group">
      ...
      <button v-if="contactLocal.id" type="button" class="ml-2 btn btn-danger" @click="deleteContact">Xóa</button>
...
export default {
  ...
  data() {
    const contactFormSchema = yup.object().shape({
      name: yup.string().required("Tên phải có giá trị.").min(2, "Tên ít nhất 2 ký tự.").max(50, "Tên tối đa 50 ký tự."),
      email: yup.string().email("E-mail không hợp lệ.").max(50, "E-mail tối đa 50 ký tự."),
      address: yup.string().max(100, "Địa chỉ tối đa 100 ký tự."),
      phone: yup.string().matches(/((09|03|07|08|05)+([0-9]{8})\b)/g, "Số điện thoại không hợp lệ."),
    });

    return {
      contactLocal: { ...this.contact }, // Sao chép đối tượng contact để tránh sửa đổi props trực tiếp
      contactFormSchema,
    };
  },
  methods: {
    ...
    cancel() {
      if (confirm("Bạn có thay đổi chưa lưu! Bạn có muốn thoát?")) {
